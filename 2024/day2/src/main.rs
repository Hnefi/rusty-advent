...
fn get_levels_from_report(s: &String) -> Vec<u32> {
    let levels: Vec<u32> = s.split_whitespace()
...
fn generate_dampened_reports(original_report: &String) -> Vec<Vec<u32>> {
    ...
    let levels = get_levels_from_report(original_report);
...
fn main() {
    ...
    println!("Hello AOC 2024 Day 2!! Calculating list safety from file {}...", fname);
